...
jQuery( document ).ready( function() {
	jQuery('#sideF').css('postion','fixed');	
...
var i=0;
	...
	for (var i = 0; i < obj.length; i++) {
		...
		if(startD==start){
			//alert("oui");
			jQuery('#checkF').hide();
			jQuery('#messageF').text(obj[i].descritpion);
		}
	  }
	        ...
	        success: function(json) {
				//alert('OK'); // le message s'affiche 
				 var myJSON = JSON.stringify(json);
				  ...
				  //alert(start);
					var nn = jQuery.fullCalendar.moment(start,'DD/MM/YYYY').format('L');
				      //nn= new Date(nn);
 ...
 var calendar=jQuery('#calendar-holder').fullCalendar({
		...
		select: function(start, end, allDay,jsEvent ) { 
			var temps = jQuery.fullCalendar.moment(start,'DD/MM/YYYY').format('L');
			var dayEv   = jQuery.fullCalendar.moment(start,'DD/MM/YYYY').format('E');
			var currD= new Date();

			
			if(dayEv==7 || start< currD){
				jQuery('#formEventCustomC').modal('hide');

			}else{
				jQuery('#formEventCustomC').modal('show');
...
//		    history.pushState(temps,url);
			...
			jQuery().modal()
		       ...
		       var Estart = jQuery.fullCalendar.moment(start).format('DD/MM/YYYY HH:mm:ss');
				var Eend = jQuery.fullCalendar.moment(end).format('DD/MM/YYYY HH:mm:ss');
				jQuery(".datetimepicker").attr('placeholder',Estart);
				jQuery(".datetimepicker1").attr('placeholder',Eend);
				jQuery(".datetimepicker").val(Estart);
				jQuery(".datetimepicker1").val(end);
				...
				events: function(start, end, timezone, callback) {
						jQuery.ajax({
							...
							success: function (doc) {
								  ...
								  var obj = JSON.parse(myJSON);
								  //alert(obj);
								  jQuery.each(obj, function(j, objs) {

								  //for(var i=0;i<obj.length;i++){
								  //var objs= obj.title;
								  //alert(objs.title);
											  
										var startD= jQuery.fullCalendar.moment(objs.startDate,'YYYY-MM-DD').format('L');
											var Dt= jQuery.fullCalendar.moment.parseZone(objs.startDate,'YYYY-MM-DD H:mm').format('LT');
												Dt= Dt.toString();
											     // alert(startD);
										   ...
										   var endD =  jQuery.fullCalendar.moment(objs.endDate,'YYYY-MM-DD').format('L');
										   var endDt =  jQuery.fullCalendar.moment.parseZone(objs.endDate,'YYYY-MM-DD H:mm').format('LTS');
										   //endD= endD.toString();
...
//#formEventCustomC  
 ...
 var calendar=jQuery('#calendar-holder').fullCalendar('today').addClass('.fc-today-green');
